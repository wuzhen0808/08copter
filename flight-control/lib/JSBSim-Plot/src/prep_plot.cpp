...
int prep_plot_main(int argc, char **argv) {
    ...
    for (int i = 2; i < argc; i++) {
        ...
        if (input_arg.substr(0, 6) == "--plot") {
            ...
            cerr << endl
                 << "Unknown argument " << input_arg << endl;
            exit(-1);
        }
    }
    ...
    if (comprehensive) {
        ...
        for (unsigned int i = 1; i < num_names; i++) {
            ...
            if (i <= num_names - 3 &&
                ((
                     names[i].find("_X") != string::npos && names[i + 1].find("_Y") != string::npos && names[i + 2].find("_Z") != string::npos) ||
                 (names[i].find("X_") != string::npos && names[i + 1].find("Y_") != string::npos && names[i + 2].find("Z_") != string::npos) ||
                 (names[i].find("P ") != string::npos && names[i + 1].find("Q ") != string::npos && names[i + 2].find("R ") != string::npos)))
    ...
    for (int fl = 0; fl < plotspecfiles.size(); fl++) {
        ...
        if (!plotDirectivesFile) {
            cerr << "Could not open autoplot file " << plotspecfiles[fl] << endl
                 << endl;
...
string HaveTerm(const vector<string> &names, const string parameter) {
    for (unsigned int i = 0; i < names.size(); i++) {
        if (names[i].find(parameter) != string::npos) {
            ...
            if (start + parameter.length() == names[i].size())
                return names[i];
        }
    }
...
bool MakeArbitraryPlot(
    ...
    if (myPlot.plotType == points)
        plotType = "points";

    // This line assumes time is in column 1
    if (GetTermIndex(names, XAxisName) == 1)
        time_range = plot_range;

    have_all_terms = have_all_terms && !HaveTerm(names, XAxisName).empty();
    for (i = 0; i < numLeftYAxisNames; i++)
        have_all_terms = have_all_terms && !HaveTerm(names, LeftYAxisNames[i]).empty();
    for (i = 0; i < numRightYAxisNames; i++)
        have_all_terms = have_all_terms && !HaveTerm(names, RightYAxisNames[i]).empty();
    ...
    if (have_all_terms) {
        ...
        if (myPlot.plotType == points)
            newPlot << "set pointsize 0.25" << endl;
        ...
        if (!multiplot) { // ## Single file plotting ##
            ...
            for (int f = 0; f < files.size(); f++) {
                ...
                if (HaveTerm(NamesArray[f], XAxisName).empty())
                    HasAllTerms = false;
                for (unsigned int c = 0; c < numLeftYAxisNames; c++)
                    if (HaveTerm(NamesArray[f], LeftYAxisNames[c]).empty())
                        HasAllTerms = false;
                for (unsigned int c = 0; c < numRightYAxisNames; c++)
                    if (HaveTerm(NamesArray[f], RightYAxisNames[c]).empty())
                        HasAllTerms = false;
...
void EmitComparisonPlot(const vector<string> &filenames, const int index, const string linetitle) {
    ...
    if (!fail) {
        ...
        cout << "##" << endl
             << "##" << endl;
        cout << "plot " << plot_range << " \"" << filenames[0] << "\" using 1:" << GetTermIndex(NamesArray[0], varname) << " with lines title \"" << linetitle << ": 1"
             << "\", \\" << endl;
